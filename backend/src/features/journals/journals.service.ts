...
export class JournalService {
  ...
  private toJournalResponse(row: JournalRow, analysis: AnalysisRow | null): JournalResponse {
    ...
    return {
      ...
      summary: analysis ? "Development mock analysis is available. It is not a clinical assessment." : "No analysis result is available.",
  ...
  async analyze(userId: string, journalId: string): Promise<AnalysisResponse> {
    ...
    const { data: consent, error: consentError } = await this.database
      ...
      .limit(1)
      .maybeSingle();
    if (consentError) throw new ExternalServiceError("DATABASE_UNAVAILABLE", "The consent check is temporarily unavailable.");
    ...
    const { data: pending, error: pendingError } = await this.database
      ...
      .select("*")
      .single();
    if (pendingError || !pending) throw new ExternalServiceError("DATABASE_UNAVAILABLE", "The analysis request could not be created.");
  ...
  private toAnalysisResponse(row: AnalysisRow): AnalysisResponse {
    return {
      ...
      risk_indication: row.urgent_language_detected === true ? "Development urgent-language fixture flag" : "Development fixture result",
      is_demo_data: true,
      created_at: asString(row.created_at),
