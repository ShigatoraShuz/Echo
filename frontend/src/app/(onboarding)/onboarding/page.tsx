...
export default function OnboardingPage() {
  ...
  const [preferredCheckInTime, setPreferredCheckInTime] = useState("20:30");
  const [startingMood, setStartingMood] = useState<"calm" | "happy" | "neutral" | "sad" | "anxious" | "angry">("calm");
  ...
  useEffect(() => {
    service.getStatus().then((result) => {
      ...
      setBuddyTone(result.data.buddyTone ?? "gentle");
      setPreferredCheckInTime(result.data.preferredCheckInTime ?? "20:30");
      setStartingMood((result.data.startingMood as typeof startingMood) ?? "calm");
          ...
          {step === 0 && (
            <div className="mx-auto max-w-xl">
              ...
              <fieldset className="mt-6">
                <legend className="text-sm font-bold">What would you like ECHO to support?</legend>
                <div className="mt-3 flex flex-wrap gap-2">
                  {["daily_reflection", "mood_awareness", "gentle_routines", "stress_support"].map((goal) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => setGoals((current) => selected ? current.filter((item) => item !== goal) : [...current, goal])}
                        className={`min-h-11 rounded-full border px-4 text-sm font-bold ${selected ? "border-[#526f35] bg-[#526f35] text-white" : "border-[#526f3530]"}`}
                      >
                        {goal.split("_").map((word) => `${word[0]?.toUpperCase()}${word.slice(1)}`).join(" ")}
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-3">
                <label className="text-sm font-bold">
                  Buddy tone
                  <select value={buddyTone} onChange={(event) => setBuddyTone(event.target.value)} className="auth-wizard-input mt-2">
                ...
                <label className="text-sm font-bold">
                  Check-in time
                  <input type="time" value={preferredCheckInTime} onChange={(event) => setPreferredCheckInTime(event.target.value)} className="auth-wizard-input mt-2" />
                </label>
                <label className="text-sm font-bold">
                  Starting vibe
                  <select value={startingMood} onChange={(event) => setStartingMood(event.target.value as typeof startingMood)} className="auth-wizard-input mt-2">
                    {(["calm", "happy", "neutral", "sad", "anxious", "angry"] as const).map((mood) => <option key={mood} value={mood}>{mood}</option>)}
                  </select>
                </label>
              </div>
