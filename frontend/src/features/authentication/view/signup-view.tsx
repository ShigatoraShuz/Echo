...
import { ArrowLeft, ArrowRight, CalendarDays, Check, Eye, EyeOff, FileText, Leaf, Loader2, Mail } from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      {normalizedContent.split(/\n\n+/).map((block, index) =>
        block.startsWith("## ") ? (
          <h3 key={index} className="text-2xl text-[#203326] [font-family:var(--font-echo-display)]">
            {block.slice(3)}
          </h3>
        ) : (
          <p key={index} className="text-sm leading-7 text-[#566057]">
            {block.replace(/\n/g, " ")}
          </p>
        ),
      )}
...
export function SignupView({ title, description }: SignupViewProps) {
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      const node = dialogScroll.current;
      node?.focus();
      if (node && node.scrollHeight <= node.clientHeight + 8) {
        setReviewed((current) => (current.includes(activePolicy.id) ? current : [...current, activePolicy.id]));
      }
    });
    return () => {
  ...
  return (
    <div className="w-full max-w-[40rem] py-4 text-[#263226] [font-family:var(--font-echo-sans)]">
      <header className="mb-7">
        <div className="flex items-center gap-3">
          <span className="grid size-11 place-items-center rounded-2xl bg-[#526f35] text-white">
      ...
      <section
        aria-live="polite"
        className="rounded-[2rem] border border-[#526f3524] bg-white/75 p-5 shadow-[0_24px_70px_rgba(38,50,38,.10)] backdrop-blur sm:p-8"
      >
        {error && (
        ...
        {step === 1 && (
          <div>
            ...
            <h2 className="mt-2 text-3xl [font-family:var(--font-echo-display)]">Know what you&apos;re agreeing to</h2>
            <p className="mt-2 text-sm leading-6 text-[#697168]">
            ...
            <div className="mt-6 grid gap-3">
              {policies.map((policy) => {
                ...
                return (
                  <article
                    key={policy.id}
                    className="flex items-center gap-4 rounded-2xl border border-[#526f3525] bg-[#fffdf8] p-4"
                  >
      ...
      {activePolicy && (
        <div className="fixed inset-0 z-[600] grid place-items-center bg-[#122018a8] p-3 backdrop-blur-sm">
          <section
            role="dialog"
            aria-modal="true"
            aria-labelledby="policy-title"
            className="flex max-h-[90svh] w-full max-w-2xl flex-col overflow-hidden rounded-[2rem] bg-[#fffdf7] shadow-2xl"
          >
            <header className="border-b border-[#526f3520] p-5 sm:p-6">
              <h2 id="policy-title" className="text-3xl [font-family:var(--font-echo-display)]">
                {activePolicy.title}
              </h2>
              <p className="mt-1 text-xs text-[#697168]">Version {activePolicy.version}</p>
            </header>
            <div
              ref={dialogScroll}
              tabIndex={-1}
              onScroll={(event) => {
                ...
                if (node.scrollTop + node.clientHeight >= node.scrollHeight - 8 && !reviewed.includes(activePolicy.id))
                  setReviewed((current) => [...current, activePolicy.id]);
              }}
              className="min-h-0 flex-1 overflow-y-auto p-5 outline-none sm:p-7"
            >
              <PolicyBody content={activePolicy.sanitized_markdown} />
              <div className="mt-8 rounded-2xl bg-[#edf2e5] p-4 text-sm font-bold text-[#526f35]">
                End of document · {reviewed.includes(activePolicy.id) ? "Reviewed" : "Scroll here to complete review"}
              </div>
            </div>
            <footer className="border-t border-[#526f3520] p-4">
              <button
                onClick={() => setActivePolicy(null)}
                disabled={!reviewed.includes(activePolicy.id)}
                className="flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[#526f35] font-bold text-white disabled:opacity-50"
              >
