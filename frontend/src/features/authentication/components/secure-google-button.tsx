  ...
  useEffect(() => {
    ...
    async function prepare() {
      try {
          ...
          try {
            if (intent === "login") {
              ...
              if (check.status === "password_account_requires_link")
                throw new Error("This email uses password login. Sign in with your email and password.");
        ...
        setStatus("ready");
        // The User Service login proof expires after five minutes. Request an
        ...
        setError(
          reason instanceof TypeError
            ? "ECHO could not reach its sign-in service. Check that the API Gateway is running, then retry."
