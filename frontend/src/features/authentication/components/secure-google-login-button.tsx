...
export function SecureGoogleLoginButton() {
  ...
  async function start() {
    if (!clientId || !window.google) return setError("Google login is not configured yet.");
    setBusy(true);
    setError("");
    try {
      const challenge = await registrationApi.googleLoginNonce();
      ...
      setBusy(false);
      setError(reason instanceof Error ? reason.message : "Google login could not start.");
    }
  }
  return (
    <>
      <button
        ...
        className="flex h-12 w-full items-center justify-center gap-3 rounded-full border border-[#526f3540] bg-white font-bold disabled:opacity-50"
      >
