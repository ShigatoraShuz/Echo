...
export function AppSidebar({
  ...
  return (
    <>
      ...
      {isMobileOpen && (
        <div 
          className="fixed inset-0 z-[250] bg-[#0d211a]/40 backdrop-blur-sm lg:hidden" 
      ...
      >
        <div className="relative flex h-20 lg:h-24 shrink-0 items-center px-6">
          <Link href="/dashboard" className="flex items-center gap-4 overflow-hidden">
            ...
            <span className={`text-xl lg:text-2xl font-black tracking-tighter transition-all duration-300 ${(isExpanded || isMobileOpen) ? "opacity-100" : "opacity-0 invisible"}`}>
              ECHO
            </span>
          </Link>
          ...
          {appLinks.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className={`ml-4 text-sm font-bold transition-all duration-300 ${(isExpanded || isMobileOpen) ? "opacity-100" : "opacity-0 invisible"}`}>
                  {item.label}
                </span>
              </Link>
            );
          })}
          <div className="pt-6"><ReflectionCalendarModal collapsed={!isExpanded && !isMobileOpen} /></div>
        </nav>

        <div className="shrink-0 px-4 pb-8 lg:pb-12 space-y-4">
          <Link href="/crisis" className="flex h-12 items-center px-4 text-[#ff9b8d] hover:bg-red-500/10 rounded-2xl transition-all">
            <ShieldAlert className="h-5 w-5 shrink-0" />
            <span className={`ml-4 text-[11px] font-black uppercase tracking-widest transition-all ${(isExpanded || isMobileOpen) ? "opacity-100" : "opacity-0 invisible"}`}>Crisis Support</span>
          </Link>
          <Link href="/settings/privacy" className="group flex items-center gap-4 rounded-[2rem] border border-white/10 bg-white/5 p-2 transition-all">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#fbf7ee] text-[#0d211a]"><ShieldCheck className="h-5 w-5" /></div>
            <div className={`flex flex-col transition-all duration-300 ${(isExpanded || isMobileOpen) ? "opacity-100" : "opacity-0 invisible"}`}>
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="relative min-h-screen bg-[#fcfaf6]">
      <div className="fixed inset-0 z-0"><RadialBackground /></div>
...
export function AuthLinkCard({ href, title, description }: { href: string; title: string; description: string }) {
  return (
    <Link href={href} className="flex items-center justify-between gap-4 rounded-2xl border border-border/70 bg-background p-4 text-sm shadow-subtle hover:bg-muted">
      <span><span className="block font-semibold text-foreground">{title}</span><span className="mt-1 block text-muted-foreground">{description}</span></span>
...
export function FloatingActionButton({ href, label }: { href: string; label: string }) {
  return (
    <Link href={href} className="fixed bottom-6 right-6 lg:bottom-10 lg:right-10 flex items-center gap-2 rounded-full bg-emerald-900 px-6 py-3 text-sm font-bold text-white shadow-2xl z-50 transition-transform active:scale-95">
...
export function PublicShell({ children }: { children: ReactNode }) {
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <EchoMarketingFooter variant="landing" />
    </div>
  );
}
