-- Prefer provider-supplied Google names when bootstrapping user_service.profiles.
...
declare
  ...
  recommended_display_name text := coalesce(
...
begin
  ...
  values (new.id, recommended_display_name)
  on conflict (user_id) do update
    set display_name = case
      when btrim(user_service.profiles.display_name) = '' then excluded.display_name
      else user_service.profiles.display_name
    end;
