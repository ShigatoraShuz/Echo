...
|---|---|---|---|---|---|---|---|---|---|
| ECHO-001 | P0 | BE | VERIFIED | — | backend/src/features/journal/journal.routes.ts, backend/src/index.ts (both fixed, then quarantined under ECHO-002) | Typecheck+build exit 0 after syntax fixes | npm run typecheck, npm run build, npm run test (20/20) | None | 2026-08-15 |
| ECHO-002 | P0 | BE | VERIFIED | 001 | backend/src-legacy/ (17 files quarantined), backend/tsconfig.json, backend/src-legacy/README.md | One entry (server.ts), one app (app.ts), one v1 router; no req.supabase leaks in src; legacy JWT decoder retired | typecheck+build exit 0; vitest 7 files/20 tests pass | None | 2026-08-15 |
| ECHO-003 | P0 | BE | VERIFIED | 002 | backend/src-legacy/shared/validation/schemas.ts moved; docs/backend/canonical-architecture.md created | One canonical implementation per concern; imports + API paths documented | typecheck+build exit 0; vitest 20/20 | None | 2026-08-15 |
