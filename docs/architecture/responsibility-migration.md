...
| Onboarding, profile, settings, consent, trusted contacts, export/deletion requests | User Service `/onboarding/*` and `/settings/*`. Profile photo bytes use the dedicated private-Storage `PUT /settings/avatar` contract; profile PATCH never transports image data. Signup display-name persistence now also uses Gateway → User Service. |
...
| Buddy sessions/messages/history, grounding, support catalog | Wellness Service. Selected Buddy conversation IDs are owner-scoped for reads and sends; Buddy verification and audit interactions use User Service APIs. |
| Dashboard and emotion aggregation | Insights Service, composed from User and Journal APIs with no direct table grants. Dashboard and emotion requests honor the explicit 7/30/90-day range. |
| CBT/wellbeing/urgent support selection | Recommendation Service public/internal recommendation endpoints, with read-only verified `support_resources` context through its dedicated database role. |
