...
export class RegistrationService {
  ...
  async acceptAgreements(
    ...
    if (draft.state !== "agreements" && draft.state !== "account") {
      throw new ConflictError("SIGNUP_STEP_REQUIRED", "Agreements cannot change after account submission.");
    }
  ...
  async registerEmail(token: string, csrf: string, email: string, password: string): Promise<DraftCredentials> {
    ...
    if (error) {
      await this.database.rpc("echo_registration_update_draft", {
        draft_id: draft.id,
        expected_token_hash: draft.token_hash,
      ...
      throw new ConflictError("EMAIL_SIGNUP_FAILED", error.message);
    }
  ...
  async bindGoogleSignup(token: string, csrf: string, idToken: string, nonce: string) {
    ...
    const { identity, status } = await this.googleLoginStatus(idToken, nonce);
    if (status !== "no_existing_account") {
      throw new ConflictError("ACCOUNT_EXISTS", "An account already exists. Use its existing sign-in method.");
    }
    const reservation = randomUUID();
