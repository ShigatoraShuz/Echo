...
async function harness() {
  ...
  const { createUserApp } = await import("../../../app.js");
  ...
  const app = createUserApp({
    ...
    database: { from: vi.fn() },
  }, { internalToken: "test-user-service-token-that-is-long-enough", allowedOrigin: origin });
  return { app, agent: request.agent(app), service, verifyIdentity };
}

function localPost(agent: ReturnType<typeof request.agent>, path: string) {
  return agent.post(path).set("Origin", origin).set("X-Forwarded-Proto", "http").set("X-Forwarded-Host", "127.0.0.1:3000");
}
...
describe("Google login challenge cookie transport", () => {
  ...
  it("round-trips a real signed challenge using HTTP cookies on direct development loopback", async () => {
    ...
    const challenge = await localPost(agent, `${prefix}/login-nonce`).send({});
    ...
    const confirmation = await localPost(agent, `${prefix}/login-challenge`)
    ...
    const result = await localPost(agent, `${prefix}/login-status`)
  ...
  it("keeps two pending login attempts valid without overwriting either challenge", async () => {
    const { agent, verifyIdentity } = await harness();
    const first = await localPost(agent, `${prefix}/login-nonce`).send({});
    const second = await localPost(agent, `${prefix}/login-nonce`).send({});
  ...
  it("always retains Secure in production, including loopback origins", async () => {
    ...
    const result = await localPost(agent, `${prefix}/login-nonce`).send({});
  ...
  it("does not relax Secure for a non-loopback hostname in development", async () => {
    const { agent } = await harness();
    const result = await agent.post(`${prefix}/login-nonce`).set("Origin", origin).set("X-Forwarded-Proto", "http").set("X-Forwarded-Host", "echo.example").send({});
    expect(result.headers["set-cookie"][0]).toContain("Secure");
  });
