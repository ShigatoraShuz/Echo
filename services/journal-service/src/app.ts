...
const input = z.object({ title: z.string().trim().min(1).max(200), body: z.string().trim().min(1).max(20_000), mood: z.enum(["calm", "happy", "neutral", "sad", "anxious", "angry"]), emotions: z.array(z.string().min(1).max(80)).max(20).default([]), tags: z.array(z.string().min(1).max(80)).max(20).default([]), privacyStatus: z.enum(["private", "shared"]).default("private"), analysisConsent: z.boolean().default(false) });
...
function parse<T>(schema: z.ZodType<T>, value: unknown): T { const result = schema.safeParse(value); if (!result.success) throw new ServiceError(400, "VALIDATION_ERROR", "The request is invalid.", result.error.flatten()); return result.data; }
...
export function createJournalApp(service: JournalService, options: { internalToken: string; allowedOrigin?: string }) {
  ...
  router.get("/journals", user, asyncRoute(async (req, res) => sendData(res, { entries: await service.list(req.auth!.id) }, req.requestId)));
